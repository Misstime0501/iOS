...
/**
 *  这个结构体只有一个属性，即指向其类的 isa 指针。
 *  这样，当我们向一个 Objective-C 对象发送消息时，runtime 库会根据实例对象的 isa 指针找到这个实力对象所属的类。
 *  runtime 库会在类的方法列表及弗雷德方法列表中寻找与消息对应的 selector 指向的方法。找到后即运行这个方法。
 ***************************************************************************************************
 *  当创建一个特定类的实例对象时，分配的内存包含一个 objc_object 数据结构，然后是类的实例变量的数据。 
 *  NSObject 类的 alloc 和 allocWithZone: 方法使用函数 class_createInstance 来创建 objc_object 数据结构。
 ***************************************************************************************************
 *  另外还有我们常见的 id ,它是一个 objc_object 结构类型的指针。它的存在可以让我们实现类似于 C++ 中泛型的一些操作。
 *  该类型的对象可以转换为任意一种对象，有点类似于 C 语言中的 void * 指针类型的作用。
 */
#endif
