...
// Objective-C 类是有 Class 类型来表示的，它实际上是一个指向 objc_class 结构体的指针，定义如下：
// typedef struct objc_class * Class;
// 可见 objc.h
struct objc_class {
    // 注意，在 Objective-C 中，所有的类自身也是一个对象，这个对象的 Class 里也有一个 isa 指针，它指向 metaClass (元类)
    Class isa  OBJC_ISA_AVAILABILITY;
#if !__OBJC2__
    // 指向该类的父类，如果该类已经是最顶层的根类（如 NSObject 或 NSProxy ）,则该值为 NULL
    Class super_class                                        OBJC2_UNAVAILABLE;     // 父类
    
    const char *name                                         OBJC2_UNAVAILABLE;     // 类名
    
    // 我们可以使用这个字段来提供类的版本信息，这对于对象的序列化非常有用，
    // 它可是让我们识别出来不同类定义版本中实例变量布局的改变。
    long version                                             OBJC2_UNAVAILABLE;     // 类的版本信息，默认为 0
    long info                                                OBJC2_UNAVAILABLE;     // 类信息，供运行期使用的一些位标识
    long instance_size                                       OBJC2_UNAVAILABLE;     // 该类实例变量大小
    struct objc_ivar_list *ivars                             OBJC2_UNAVAILABLE;     // 该类的成员变量链表
    struct objc_method_list **methodLists                    OBJC2_UNAVAILABLE;     // 方法定义的链表
    
    // 用于缓存最近使用的方法。一个接收者对象接收到一个消息时，它会根据 isa 指针去查找能够响应这个消息的对象。
    // 在实际使用中，这个对象只有一部分方法是常用的，很多方法其实很少用或者根本用不上。
    // 这种情况下，如果每次消息来时，我们都是 methodLists 中遍历一遍，性能势必很差。这是， cache 就派上了用场。
    // 在我们每次调用过一个方法后，这个方法就会被缓存到 cache 列表中，下次调用的时候 runtime 就会优先去 cache 中找，
    // 如果 cache 没有，才去 methodList 中查找方法。这样对于那些经常用到的方法的调用，提高了调用的效率。
 ...
 *      2. 检测 NSObject 是否相应 +alloc 方法，发现响应，于是检测 NSArray 类，并根据其所需要的内存空间大小开始分配内存空间，
 *         然后把 isa 指针指向 NSArray 类。同时， +alloc 也被加进 cache 列表里面。
 *      3. 接着，执行 -init 方法，如果 NSArray 响应该方法，则直接将其加入 cache 列表里面；如果不响应，则去父类查找。
 *      4. 在后期的操作中，如果再以 [[NSArray alloc] init] 这种方式来创建数组，则会直接从 cache 列表中去除相应的方法，直接调用。
 *
 */
 ...
 *  meta-class 是一个类对象的类。
 *
 *  当我们想一个对象发送消息时， runtime 会在这个对象所属的这个类的方法列表中查找方法；而向一个类发送消息时，会在这个类的 meta-class 的方法列表中查找。
 *
 *  meta-class 之所以重要，是因为它存着一个类的所有类方法。每个类都会有一个独立的 meta-class ，因为每个类的类方法基本不可能完全相同。
 *
 *  meta-class 也是一个类，也刻意向它发送一个消息，那么它的 isa 又指向什么呢？ 
 *  Objective-C 的设计者让所有的 meta-class 的 isa 自相基类的 meta-class ，以此作为他们的所属类。
 *  即，任何 NSObject 继承体系下的 meta-class 都使用 NSObject 的 meta-class 作为自己的所属类，而积累的 meta-class 的 isa 指针指向它自己。
 *
 *  对于 NSObject 继承体系来说，其实例方法对体系中的所有实例、类和 meta-class 都是有效的；而类方法对于体系内所有类和 meta-class 都是有效的。
 *
 ***************************************************************************************************
 *  示例请看 MetaClassTest 类
 *
 *  这个例子是在运行时创建了一个 NSError 的子类 TestClass ，然后为这个子类添加一个方法 testMetaClass ，这个方法的实现是 TestMetaClass 函数。
 *  运行后打印结果
 *  2016-06-16 15:41:42.765 RuntimeDemo[3730:420512] This object is 0x7ff142f21390
 *  2016-06-16 15:41:42.766 RuntimeDemo[3730:420512] Class is TestClass, super class is NSError
 *  2016-06-16 15:41:42.766 RuntimeDemo[3730:420512] Following the isa pointer 0 times gives 0x7ff142f22b00
 *  2016-06-16 15:41:42.766 RuntimeDemo[3730:420512] Following the isa pointer 1 times gives 0x0
 *  2016-06-16 15:41:42.766 RuntimeDemo[3730:420512] Following the isa pointer 2 times gives 0x0
 *  2016-06-16 15:41:42.766 RuntimeDemo[3730:420512] Following the isa pointer 3 times gives 0x0
 *  2016-06-16 15:41:42.766 RuntimeDemo[3730:420512] NSObject's class is 0x1042d4170
 *  2016-06-16 15:41:42.766 RuntimeDemo[3730:420512] NSObject's meta class is 0x0
 ***************************************************************************************************
 *
 *  我们在 for 循环中，我们通过 objc_getClass 来获取对象的 isa ，并将其打印出来，依此一直回溯到 NSObject 的 meta-class 。
 *  从打印结果，可以看到最后指针指向的地址是 0x0 ， 即 NSObject 的 meta-class 的类地址。
 **
 *  需要注意：我们在一个类对象调用 class 方法是无法获取 meta-class ，它只是返回类而已。
 **
 */
#pragma mark -
...
#define CLS_EXT                 0x20000
    ...
    // Ivar 是 runtime 对变量的定义，本质是一个结构体：
    // 此处参考 http://www.jianshu.com/p/fed1dcb1ac9f
    /**
     ivar_name: 变量名，对于一个给定的 Ivar , 可以通过 const char *ivar_getName(Iver v) 函数获得 char * 类型的变量名。
     ivar_type: 变量类型，在 runtime 中变量类型用字符串表示，例如用 @ 表示 id 类型，用 i 表示 int 类型。类似的，可以通过 const char *ivar_getTypeEncoding(Ivar v) 函数获得变量类型。
     ivar_offset: 基地址偏移字节数，可以不予理会。
...
#define CACHE_HASH(sel, mask) (((unsigned int)((uintptr_t)(sel)>>3)) & (mask))
    ...
    // objc_class 结构体重的 cache 字段，用于缓存调用过的方法，其定义如下：
    struct objc_cache {
    ...
    /**
     *  该结构体的秒如如下：
     *      1. mask: 一个整数，指定分配的缓存 bucket 的总数。在方法查找过程中， Objective-C runtime 使用这个字段来确定开始线性查找数组的索引位置。
     *               指向方法 selector 的指针与该字段做一个 AND 位操作（index = (mask & selector)）。这可以作为一个简单的 hash 散列算法。
     *      2. occupied: 一个整数，指定实际占用的缓存 bucket 的总数。
     *      3. buckets: 指向 Method 数据结构指针的数组。这个数组可能包含不超过 mask+1 个元素。
     *                  需要注意的是，指针可能是 NULL ，表示这个缓存 bucket 没有被占用，另外被占用的 bucket 可能是不连续的。
     *                  这个数组可能会随着时间而增长。
     */
